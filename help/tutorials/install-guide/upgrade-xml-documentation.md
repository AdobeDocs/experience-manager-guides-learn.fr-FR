...
Si vous utilisez AEM Guides version 3.8.5, vous pouvez effectuer la mise à niveau vers la version 4.0 de AEM Guides. Avec la fonction de mise à niveau, vous n’avez pas à désinstaller la version précédente d’AEM Guides.

Avant d’exécuter le processus, vous devez effectuer certaines tâches. Les sous-sections suivantes décrivent les conditions préalables, la génération de rapports et le processus de migration. En outre, après avoir installé AEM Guides version 4.0, vous pouvez personnaliser différentes configurations, en fonction des paramètres du client.
...
Avant de lancer le processus de mise à niveau AEM Guides, vérifiez que vous disposez des éléments suivants :
...
1. Vérifiez qu’à ce stade AEM Guides est sur la version 3.8.5.
...
1. Si un dispatcher est configuré sur l’instance d’auteur AEM, procédez comme suit :
   ...
   - Le modèle d’URL /home/users/\*/preferences est placé sur la liste autorisée.
...
>[!NOTE]
>
> Le modèle dam-update-asset peut être personnalisé. Ainsi, si des personnalisations ont été effectuées, nous devons synchroniser les personnalisations et les AEM Guides dans la copie de travail du modèle.
...
1. Cliquez sur **Insérer le composant** \(Responsable du post-traitement des Guides d’AEM comme étape finale du processus\).
...
1. Cliquez sur **Synchronisation** en haut à droite après avoir terminé les modifications. Vous recevrez une notification de succès.
   ...
   >[!NOTE]
   >
   > Actualisez et vérifiez que les modifications personnalisées et l’étape de post-traitement AEM Guides sont présentes dans le modèle de workflow final.
...
La mise à niveau vers la version 4.2.1 dépend de la version actuelle des AEM Guides.
