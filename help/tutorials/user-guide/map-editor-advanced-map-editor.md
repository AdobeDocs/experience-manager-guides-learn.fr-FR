...
## Ajout de rubriques à un fichier de mappage
...
1. Cliquez sur l’icône **Modifier**.
   ...
   - **C** - \(*Mapper les vues*\) : Permet de basculer l’éditeur de cartes entre la mise en page, l’auteur, la source et l’aperçu. Le **Disposition** vous permet d’organiser les rubriques dans un mappage DITA. Cela donne l’arborescence ou la vue hiérarchique de la carte. Le **Auteur** La vue vous permet de modifier les rubriques dans l’éditeur de cartes. Cela donne également la vue WYSIWYG du fichier de carte. Le **Source** La vue vous permet de travailler avec le XML sous-jacent du fichier map. L’aperçu vous donne une vue consolidée de toutes les rubriques et sous-mappages dans le fichier de mappage. Le **Fermer** ferme le fichier map .

   - **D** - \(*Panneau gauche*\) : Permet d’accéder au panneau de gauche qui vous donne accès aux fonctionnalités Favoris, Référentiel, Carte, Contour et autres. Vous pouvez la développer ou la réduire en cliquant sur l’icône Développer la barre latérale \(![](images/sidebar-expand-icon.svg)\). Pour plus d’informations sur les fonctionnalités disponibles dans le panneau de gauche, voir [Panneau gauche](web-editor-features.md#id2051EA0M0HS) dans l’éditeur Web.

   - **E** - \(*Centre*\) : Zone de modification du contenu de la carte.
...
## Mode Disposition {#id205DEC0005Z_layout_view}
...
>[!NOTE]
>
> La vue Disposition affiche uniquement les références présentes dans une carte. Si des références sont rompues, un petit symbole croisé s’affiche à gauche de la référence.
...
Insérez le `topicgroup` élément . Pour plus d’informations sur le regroupement de rubriques, voir [topicgroup](https://docs.oasis-open.org/dita/v1.0/langspec/topicgroup.html) documentation dans la spécification du langage OASIS DITA.
...
Vérifiez les versions et les libellés disponibles sur votre principale rubrique, puis revenez à n’importe quelle version à partir de l’éditeur lui-même.
...
- **Déplacer**: Vous pouvez choisir de déplacer une rubrique vers le haut/vers le bas/vers la droite/vers la gauche dans la hiérarchie. Vous pouvez également faire glisser et déposer une rubrique ou une map du panneau du référentiel vers la carte ouverte dans l’éditeur de mappage.
- **Annuler**: Annulez la dernière opération dans la vue Disposition.
- **Rétablir**: Rétablissez la dernière opération dans la vue Disposition.
...
Pour plus d’informations sur les métadonnées et les attributs de rubrique standard, reportez-vous à la section [topicref](https://docs.oasis-open.org/dita/v1.2/os/spec/langref/topicref.html) documentation dans la spécification du langage OASIS DITA.
...
Le **Auteur** La vue vous permet de modifier votre mappage DITA dans l’éditeur web. La vue WYSIWYG de l’éditeur de cartes s’affiche. Certaines icônes affichées dans la vue Auteur sont identiques à la vue Mise en page. Pour plus d’informations, voir [Mode Disposition](#id205DEC0005Z_layout_view). En outre, vous pouvez voir les icônes suivantes et effectuer les tâches associées à partir de la vue Auteur :
...
1. Une fois que vous avez terminé de modifier les rubriques, vous pouvez effectuer les opérations suivantes :
...
- **Éditeur de carte avancé**: Dans l’éditeur de carte avancé, cliquez sur l’icône Aperçu pour afficher l’aperçu de la carte actuelle.
   ...
   - La rubrique actuelle dans l’aperçu de la carte est également mise en surbrillance dans l’arborescence de la carte.
